...
async def start_handler(message: types.Message):
    ...
    if not_subbed:
        ...
        await message.answer(f"Xush kelibsiz! Bo'limlardan birini tanlang.", reply_markup=main_menu_kb())
...
async def check_subs_callback(call: types.CallbackQuery):
    ...
    if not_subbed:
        ...
        await call.message.answer("Obuna tasdiqlandi!", reply_markup=main_menu_kb())
...
async def show_product(message, cat_id, index):
    ...
    builder.button(text="❌ Yopish", callback_data="back_to_cats")
    builder.button(text="➡️", callback_data=f"nav_{cat_id}_{index+1}")
    builder.adjust(3)
...
async def adm_stats(call: types.CallbackQuery):
    ...
    await call.answer(f"Foydalanuvchilar: {u} ta", show_alert=True)
...
async def save_category(message: types.Message, state: FSMContext):
    ...
    await message.answer("Qo'shildi!", reply_markup=admin_panel_kb())
    await state.clear()
...
async def adm_add_prod(call: types.CallbackQuery, state: FSMContext):
    ...
    builder.adjust(2)
    await call.message.answer("Bo'limni tanlang:", reply_markup=builder.as_markup())
...
async def addp_final(message: types.Message, state: FSMContext):
    ...
    await message.answer("Tayyor!", reply_markup=admin_panel_kb())
    await state.clear()
...
async def start_bc(message: types.Message, state: FSMContext):
    ...
    await message.answer("Yuborildi!", reply_markup=admin_panel_kb())
    await state.clear()
...
async def sub_id(message: types.Message, state: FSMContext):
    ...
    await message.answer("Kanal linki (https://...):")
...
async def on_startup(bot: Bot):
    db_start()
    await bot.delete_webhook(drop_pending_updates=True) # Eski xabarlarni o'chirib yuboradi
    await bot.set_webhook(f"{WEBHOOK_URL}{WEBHOOK_PATH}")

def main():
    dp.startup.register(on_startup)
    app = web.Application()
    app.router.add_get("/", lambda r: web.Response(text="Ishlayapti!"))
